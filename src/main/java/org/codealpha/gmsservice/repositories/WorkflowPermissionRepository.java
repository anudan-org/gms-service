...
public interface WorkflowPermissionRepository extends CrudRepository<WorkFlowPermission, Long> {
  ...
  @Query(value = "select wst.id, wst.from_state_id, (select name from workflow_statuses where id = wst.from_state_id) from_name, wst.to_state_id, (select name from workflow_statuses where id = wst.to_state_id) to_name, wst.action, wst.note_required,wst.seq_order from workflow_status_transitions wst inner join grant_assignments c on c.state_id=wst.from_state_id where wst.from_state_id=?1 and c.assignments=?2 and c.grant_id=?3 group by (wst.id, wst.from_state_id, wst.to_state_id, wst.action) order by wst.seq_order asc", nativeQuery = true)
