...
public interface ClosureAssignmentRepository extends CrudRepository<ClosureAssignments,Long> {
    ...
    List<ClosureAssignments> findByClosureIdIn(List<Long> ids);

    @Query(value = "select a.id as \"id\", a.closure_id as \"closureId\", a.state_id as \"stateId\", a.assignment as \"assignmentId\" " +
            "from closure_assignments a where a.closure_id in (?1)", nativeQuery = true)
    List<ClosureAssignmentSummaryRow> findSummaryByClosureIdIn(List<Long> ids);
