...
public interface DisbursementRepository extends CrudRepository<Disbursement, Long> {
    ...
    @Query(value = "select id, requested_amount, reason, requested_on, requested_by, status_id, grant_id, note, note_added, note_added_by, created_at, created_by, updated_at, updated_by, moved_on, grantee_entry, other_sources, report_id, disabled_by_amendment,get_owner_disbursement_name(id) owner_name,get_owner_disbursement(id) owner_id from disbursements where grant_id=?1 and status_id in (?2) and ", nativeQuery = true)
