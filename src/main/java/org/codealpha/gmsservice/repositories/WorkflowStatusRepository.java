...
public interface WorkflowStatusRepository extends CrudRepository<WorkflowStatus,Long> {
  ...
  @Query(value = "select ws.* from workflows w inner join workflow_statuses ws on w.id = ws.workflow_id inner join grant_type_workflow_mapping m on m.workflow_id=w.id where ws.initial=true and w.object=?1 and m.grant_type_id=?3 and w.granter_id=?2 and m._default=true", nativeQuery = true)
