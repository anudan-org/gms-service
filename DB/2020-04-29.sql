...
drop materialized view granter_active_grants_summary_committed;
drop materialized view granter_active_grants_summary_disbursed;

create materialized view granter_grants_summary_committed as select A.grantor_org_id as granter_id,B.internal_status,count(A.*) grant_count,min(start_date) period_start,max(end_date) period_end,sum(amount) committed_amount from grants A
inner join workflow_statuses B on B.id=A.grant_status_id
where B.internal_status in ('ACTIVE','CLOSED')
group by A.grantor_org_id,B.internal_status;

create materialized view granter_grants_summary_disbursed as select A.grantor_org_id as granter_id,B.internal_status,D.value as disbursement_data  from grants A
inner join workflow_statuses B on B.id=A.grant_status_id
...
where E.field_type='disbursement' and F.internal_status='CLOSED' and B.internal_status in ('ACTIVE','CLOSED');
...
begin
    refresh materialized view granter_count_and_amount_totals;
	refresh materialized view granter_grantees;
	refresh materialized view granter_grants_summary_committed;
	refresh materialized view granter_grants_summary_disbursed;
end
$BODY$;