...
--INITIAL STATISTICS
select count(*) into grantsStat from grants where organization_id=69 and deleted=false;
select count(*) into grantsDraftStat from grants a inner join workflow_statuses b on b.id=a.grant_status_id where a.organization_id=69 and b.internal_status='DRAFT' and (select count(*) from grant_history where id=a.id)=0 and a.deleted=false;
select count(*) into grantsInprogressStat from grants a inner join workflow_statuses b on b.id=a.grant_status_id where a.organization_id=69 and ((b.internal_status='DRAFT' and (select count(*) from grant_history where id=a.id)>0) OR (b.internal_status='REVIEW')) and a.deleted=false;
select count(*) into grantsActiveStat from grants a inner join workflow_statuses b on b.id=a.grant_status_id where a.organization_id=69 and b.internal_status='ACTIVE' and a.deleted=false;
select count(*) into reportsStat from reports a where a.grant_id in (select id from grants where organization_id=69 and deleted=false);
select count(*) into reportsStatDraftNM from reports a inner join workflow_statuses b on b.id=a.status_id where b.internal_status='DRAFT' and (select count(*) from report_history where id=a.id)=0 and a.grant_id in (select id from grants where organization_id=69 and deleted=false);
select count(*) into reportsStatDraft from reports a inner join workflow_statuses b on b.id=a.status_id where b.internal_status='DRAFT' and (select count(*) from report_history where id=a.id)=0 and a.grant_id in (select id from grants where organization_id=69 and deleted=false);
select count(*) into reportsStatReview from reports a inner join workflow_statuses b on b.id=a.status_id where b.internal_status='REVIEW' and (select count(*) from report_history where id=a.id)=0 and a.grant_id in (select id from grants where organization_id=69 and deleted=false);
select count(*) into reportsStatApproved from reports a inner join workflow_statuses b on b.id=a.status_id where b.internal_status='CLOSED' and (select count(*) from report_history where id=a.id)=0 and a.grant_id in (select id from grants where organization_id=69 and deleted=false);
...
select count(*) into disbursementsStatDraft from disbursements a inner join workflow_statuses b on b.id=a.status_id where b.internal_status='DRAFT' and (select count(*) from disbursement_history where id=a.id)=0 and a.grant_id in (select id from grants where organization_id=69 and deleted=false);
select count(*) into disbursementsStatReview from disbursements a inner join workflow_statuses b on b.id=a.status_id where b.internal_status='REVIEW' and (select count(*) from disbursement_history where id=a.id)=0 and a.grant_id in (select id from grants where organization_id=69 and deleted=false);
select count(*) into disbursementsStatApproved from disbursements a inner join workflow_statuses b on b.id=a.status_id where b.internal_status='ACTIVE' and (select count(*) from disbursement_history where id=a.id)=0 and a.grant_id in (select id from grants where organization_id=69 and deleted=false);
select count(*) into disbursementsStatClosed from disbursements a inner join workflow_statuses b on b.id=a.status_id where b.internal_status='CLOSED' and (select count(*) from disbursement_history where id=a.id)=0 and a.grant_id in (select id from grants where organization_id=69 and deleted=false);
...
FOR grantRow IN (select * from grants where organization_id=69) LOOP
    RAISE NOTICE '-------------------------------------------------------';
    RAISE NOTICE 'PROCESSING GRANT %',grantRow.id;
    RAISE NOTICE '-------------------------------------------------------';
	--START: Update grant type for each grant
	update grants set grant_type_id=4,grant_status_id=cini_grant_status(grant_status_id,6,17),organization_id=null where id=grantRow.id;
	...
	insert into grant_snapshot (grant_id,from_state_id,from_note,moved_by,to_state_id,assigned_by,moved_on) values(grantRow.id,grantRow.grant_status_id,'Received By System to perform Grant Type switch for Direct Implementation (As per SustainPlus request)',systemUserId,systemStatusId,systemUserId,clock_timestamp());
	insert into grant_snapshot (grant_id,from_state_id,from_note,moved_by,to_state_id,assigned_by,moved_on) values(grantRow.id,systemStatusId,'Returned back to user after Grant type switch for Direct Implementation (As per SustainPlus request)',systemUserId,cini_grant_status(grantRow.grant_status_id,6,17),systemUserId,clock_timestamp());
	...
	FOR reportRow in (select * from reports where grant_id=grantRow.id) LOOP
		...
		--START: Make 2 entries for Report History and Snaphot System Receipt + Switch to New Status
			...
			insert into report_snapshot (report_id,from_state_id,from_note,moved_by,to_state_id,assigned_to_id,moved_on) values(reportRow.id,reportRow.status_id,'Received By System to perform Grant Type switch for Direct Implementation (As per SustainPlus request)',systemUserId,systemStatusId,systemUserId,clock_timestamp());
			insert into report_snapshot (report_id,from_state_id,from_note,moved_by,to_state_id,assigned_to_id,moved_on) values(reportRow.id,systemStatusId,'Returned back to user after Grant type switch for Direct Implementation (As per SustainPlus request)',systemUserId,cini_report_status(reportRow.status_id,7,16),systemUserId,clock_timestamp());
	...
	--LOOP: Loop through each report of grant
    	...
    	FOR disbursementRow in (select * from disbursements where grant_id=grantRow.id) LOOP
    		...
    		--START: Make 2 entries for Report History and Snaphot System Receipt + Switch to New Status
    			...
    			insert into disbursement_snapshot (disbursement_id,from_state_id,from_note,moved_by,to_state_id,assigned_to_id,moved_on) values(disbursementRow.id,disbursementRow.status_id,'Received By System to perform Grant Type switch for Direct Implementation (As per SustainPlus request)',systemUserId,systemStatusId,systemUserId,clock_timestamp());
    			insert into disbursement_snapshot (disbursement_id,from_state_id,from_note,moved_by,to_state_id,assigned_to_id,moved_on) values(disbursementRow.id,systemStatusId,'Returned back to user after Grant type switch for Direct Implementation (As per SustainPlus request)',systemUserId,cini_disbursement_status(disbursementRow.status_id,9,18),systemUserId,clock_timestamp());
